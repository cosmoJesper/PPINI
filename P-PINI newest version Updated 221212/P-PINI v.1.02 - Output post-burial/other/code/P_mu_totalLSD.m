...
% 
% syntax out = P_mu_totalLSD(z,h,Rc,SPhi,consts,dflag);
%
...
%
% This code modified by Greg Balco (June 2016) so that parameters are
% passed in the same way as for P_mu_total.m. No changes to the actual
% calculations. 
%
...
mflux = MuonsX(h,Rc,SPhi);%Generates muon flux at site from Sato et al. (2008) model
...
% Determine which cross-section passed. 
...
out.R = exp(interp1(log(data(:,1)),log(data(:,2)),log(x')))';
out.Eloss = exp(interp1(log(data(:,1)),log(data(:,3)),log(x')))';

...
leg_str = {'1 in 1000 of simulations';'Accepted simulations';'P^2^6/P^1^0 Ratio = 6.8'};
if numel(k) > 3
    for i=4:numel(k)
...
%         text(0.90,0.90,'c)','Units','normalized')
        ...
        if mean_age < 0
            ...
            idx = find(cum_A > 0.50);
            disp(['50% qunatile: ',num2str(ut(idx(1))), 'yr'])
            idx = find(cum_A > 0.75);
            disp(['75% qunatile: ',num2str(ut(idx(1))), 'yr'])
            idx = find(cum_A > 0.90);
            disp(['90% qunatile: ',num2str(ut(idx(1))), 'yr'])
            idx = find(cum_A > 0.95);
            disp(['95% qunatile: ',num2str(ut(idx(1))), 'yr'])
        else
            plot(t,f_t(t),'-r')
        end
        annotation('textarrow',[0.58 0.63],[0.28 0.27],'String','1.29 \pm 0.09 Ma')
        box on
        ax = gca;

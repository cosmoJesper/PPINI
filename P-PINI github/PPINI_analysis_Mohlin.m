...
site = ' ';
name = 'mc_sink_Mohlin';    % Library name 
file = 'Mohlin.xlsx';      % Excel sheet with sample information
...
NBe = table2array(T(:,2));
NBe_unc = table2array(T(:,3))*1.5;
NAl = table2array(T(:,4));
NAl_unc = table2array(T(:,5))*1.5;
depths = table2array(T(:,6));
cd ..\code

%%% Advanced section for omitting samples
k = 1:numel(NBe);
% k([1 3]) = [];
...
leg2(1) = plot(m.NBe_tot(1,1:1000:end)*F,m.NAl_tot(1,1:1000:end)*F,'.k');
hold on
leg2(2) = plot(-100,-100,'.','Color',[1 0.6 0.1]);
ax = gca;
leg2(3) = plot(ax.XLim,ax.XLim*6.75,'-k');
leg_str = {'1 in 1000 of simulations';'Accepted simulations';'Ratio = 6.75'};
if numel(k) > 3
    for i=4:numel(k)
...
for rep = 1:rep_max
    ...
    if  thres_NBe < 0.01 && thres_NAl < 0.01
        disp('Threshold criteria reached!')
        break
    end
    
    if  thres_age < 0.01 && thres_e < 0.01
        disp('Threshold criteria reached!')
        break
    end
...
%     if j == 3 && j > n_sub*n_sub-n_sub
%         xlabel('[Ma]')
%         ylabel('[g/cm2/Myr]')
%     elseif j == 1
%         ylabel('[g/cm2/Myr]')
%         set(ax,'XTick',[])
%     elseif j > n_sub*n_sub-n_sub
%         xlabel('[Ma]')
%         set(ax,'YTick', [])
%     else
%         set(ax,'XTick',[], 'YTick', [])
%     end
    ...
    figure(2)
    hold on
    plot(NBe_tot(filter)*F,NAl_tot(filter)*F,'.','Color',[1 0.6 0.1])
...
%         text(0.90,0.90,'c)','Units','normalized')
    ...
    %     annotation('textarrow',[0.57 0.52],[0.28 0.28],'String','0 \pm 103 ka')
...
for j = 1:numel(k)
    ...
    figure(2)
        hold on
        leg1(i) = plot(NBe(i)*F,NAl(i)*F,'*','Color',clr);
        
    figure(9)
        hold on
        Al_inh = hist_info{j}(5,:);
        Al_inh = Al_inh(accept);
        Be_inh = hist_info{j}(4,:);
        Be_inh = Be_inh(accept);
        plot(Be_inh,Al_inh./Be_inh,'.','Color',clr)
%         plot(Be_inh,Al_inh,'.','Color',clr)
        
        idx = randperm(length(Be_inh),n_bay);
        Be_fit(j,:) = Be_inh(idx);
        Al_fit(j,:) = Al_inh(idx);
end
...
for i = 1:n_bay
    f = fit(Be_fit(:,i),Al_fit(:,i)./Be_fit(:,i),'poly1');
    ms(i) = f.p1; 
    bs(i) = f.p2;
end
...
hold on
ax = gca;
plot(ax.XLim,mean_m*ax.XLim+mean_b,'-k')
plot(ax.XLim,[7.6 7.6],'--r')
plot(ax.XLim,(mean_m+std_m)*ax.XLim+mean_b+std_b,'--k')
plot(ax.XLim,(mean_m-std_m)*ax.XLim+mean_b-std_b,'--k')
xlabel('^1^0Be [10^4 at/g]')
ylabel('Inherited ^2^6Al/^1^0Be')
% legend([names(k);'Regression';'7.6 ratio'],'location','southwest')
% legend('boxoff')
legend([names(k);'Regression';'7.6 ratio'],'location','southeast')
xticks([0 0.5 1 1.5 2 2.5 3]*1e4)
...
% xticks([0 0.5 1 1.5 2 2.5 3]*1e4)
% xticklabels({'0','0.5','1','1.5','2','2.5','3'})
% yticks([0 0.5 1 1.5 2 2.5]*1e5)
% yticklabels({'0','5','10','15','20','25'})
% xlabel('^1^0Be [10^4 at/g]','FontSize',20)
% ylabel('^2^6Al [10^4 at/g]','FontSize',20)
...
ax.XLim = [0; 4e4]*F;
ax.YLim = [0; 4e4*7]*F;
xlabel('^1^0Be [10^5 at/g]')
ylabel('^2^6Al [10^5 at/g]')
uistack(leg2(3),'top')

...
rho_max = 3.0;                                               % Maximum rock density in source [g/cm3]
min_ero = 10;                                                % Minimum source interglacial erosion rate [m/Myr]
max_ero = 2000;                                              % Miximum source interglacial erosion rate [m/Myr]
...
ice_lengths = 20000;                                         % Maximum potential length of a glaction at site
...
rho_pool = linspace(2.5,3.0,nn);                                  % Rock density in source [g/cm3]
pluck_depth = 1*logspace(log10(pluck_min),log10(pluck_max),nn);   % Plucking depth pr. glaciation [m]
...
for i = 1:n_elev
    ...
    P_time = P_SLHL*mean(LSDout.Be);                           % Calculating 10Be production at height
    ...
    P_time = R*P_SLHL*mean(LSDout.Al);                      % Calculating 26Al production at height

...
T = readtable('Alaska2.xlsx','VariableNamingRule','preserve');
...
y_plus = (m+m_std)*xlimit+(b);
y_minus = (m-m_std)*xlimit+(b);
...
plot(NBe(k),NAl(k),'+k')
% plot(NBe,NAl,'+k','MarkerSize',3)
% i = 5;
% plot(NBe(i)+NBe_unc(i)*cos(t),NAl(i)+NAl_unc(i)*sin(t),'--k','LineWidth',1.1)

title(site)
xlabel('^1^0Be [10^3 at/g]')
ylabel('^2^6Al [10^3 at/g]')

xticks([0 0.5 1 1.5 2 2.5]*1e3)
xticklabels({'0','0.5','1','1.5','2','2.5'})
yticks([0 2 4 6 8]*1e3)
yticklabels({'0','2','4','6','8'})
ax =gca; ax.XLim = [0 2.5e3]; ax.YLim = [0 8e3];
...
simple_figure()
h = text(0.9e3,14e3,['^2^6P/^1^0P = ',num2str(R)]);
set(h,'Rotation',48);

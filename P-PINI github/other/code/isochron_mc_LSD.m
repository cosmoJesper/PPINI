...
data_all = textscan(data,'%f%f%f%f%%fs\r\n');
fclose(data);
Al = data_all{1}*1e-6;
...
%             R_post = 2;
       ...
       [b,m,sb,sm,S]  = york_fit(X',Y',sX',sY');
        ...
        Age = -taubur*log(m*tau26/tau10);
        
        %Get production rate at time zero using new age
        P_age = interp1(time,P_time,Age);
...
Mean_offset_real = Mean_offset*P*tau26*R*10^6
Mean_offset_stddev_real = offset_error*P*tau26*R*10^6
